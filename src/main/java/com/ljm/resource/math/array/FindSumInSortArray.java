...
//        System.out.println(findSumInRandomArray(randomArray, 9));
    ...
    /**
     * 非有序数组中找到和为target 的元素对应的下标
     */
    public static String findSumInRandomArray(int[] array, int target) {
        if (array == null || array.length <= 0) {
            return "-1";
        }
        Map<Integer, Integer> findMap = new HashMap<>();
        for (int i = 0; i < array.length; i++) {
            if (findMap.containsKey(array[i])) {
                System.out.println(findMap.get(array[i]) + ", " + i);
            } else {
                findMap.put(target - array[i], i);
            }
        }
        return "-1";
    }
